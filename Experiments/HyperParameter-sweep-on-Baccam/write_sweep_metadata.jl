#!/usr/bin/env julia
#
# write_sweep_metadata.jl
#
# Captures the full sweep configuration and environment into a small
# HDF5 file for reproducibility. Called once by run_sweep.sh before
# the parallel jobs launch.
#
# Usage:
...
# Load the sweep config to access grid parameters and training constants
...
function git_sha(project_dir)
    try
        strip(read(`git -C $project_dir rev-parse HEAD`, String))
    catch
        "unknown"
    end
end
...
function git_is_dirty(project_dir)
    try
        !isempty(strip(read(`git -C $project_dir status --porcelain`, String)))
    catch
        false
    end
end
...
h5open(OUTFILE, "w") do fid
    ...
    # Sweep sampling configuration
    g["N_LHS"]    = N_LHS
    g["LHS_SEED"] = LHS_SEED
    g["USE_LHS"]  = USE_LHS
    g["n_hp"]     = length(HP_GRID)

    # HP bounds (2-element [lo, hi] array per axis)
